...
- **Họ và tên**: BÙI THỊ MỸ HUỆ  

- **Mã lớp**: DK24TTC4  

- **MSSV**: 170124380

- **Email**: huebtm020184@tvu-onschool.edu.vn

- **Số điện thoại**: 0329435002 

## Cấu trúc thư mục

- Progress-report: báo cáo tiến độ

- SRC: chứa toàn bộ soucre code, database của website: Classes, Bin, Obj, Controllers, Data, Models, Properties, Views, wwwroot, appsettings.json, QuanLySanBong.csproj, Program.cs, QuanLySanBong.bak
...
## Cách cài đặt
   ...
   2. Lấy mã nguồn
    ...
    - Git clone website QLSanBongMini từ : https://github.com/huebtm020184-debug/ASPNET-DK24TTC4-BuiThiMyHue-QLSanBongMini.git về máy.

    - Mở Visual Studio: Open → ASPNET-DK24TTC4-BuiThiMyHue-QLSanBongMini/SRC/QuanLySanBong.csproj

    - Hoặc Terminal: cd ASPNET-DK24TTC4-BuiThiMyHue-QLSanBongMini\SRC
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Server=(localdb)\\MSSQLLocalDB;Database=QuanLySanBong;Trusted_Connection=True;MultipleActiveResultSets=true"
  }
}
```
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Server=.\\SQLEXPRESS;Database=QuanLySanBong;Trusted_Connection=True;TrustServerCertificate=True;MultipleActiveResultSets=true"
  }
}
```
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Server=SERVERNAME;Database=QuanLySanBong;User Id=sa;Password=your_password;TrustServerCertificate=True;MultipleActiveResultSets=true"
  }
}
```
...
---
	...
	**Visual Studio → Tools → NuGet Package Manager → Package Manager Console (PMC)**, chọn đúng project `SRC` làm *Default Project*, rồi:
	...
	- Cách B — **Restore từ file .bak**
     ...
     c. Chọn **Device** → trỏ tới file **`QuanLySanBong.bak`** trong repo → **Restore**.
...
./scripts/setup.ps1 -ProjectPath ./SRC -SqlServer "(localdb)\MSSQLLocalDB" -Database QuanLySanBong -UseWindowsAuth -Run

# Hoặc dùng tài khoản SQL
./scripts/setup.ps1 -ProjectPath ./SRC -SqlServer ".\SQLEXPRESS" -Database QuanLySanBong -User sa -Password "your_password" -Run
```
